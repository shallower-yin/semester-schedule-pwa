...
export function splitAudioForAsr(
  ...
  if (extension === "mp3") return splitMp3(bytes, maxBytes).map((chunk) => ({ bytes: chunk, mimeType: "audio/mpeg" }));
  if (extension === "wav") return splitWav(bytes, maxBytes).map((chunk) => ({ bytes: chunk, mimeType: "audio/wav" }));
  throw new Error("超过 7 MB 的音频目前仅支持 MP3、WAV 自动分段，请先转换格式后重试。");
}

function splitMp3(bytes: Uint8Array, maxBytes: number): Uint8Array[] {
  ...
  for (const frame of frames) {
    ...
    if (group.length && groupSize + frameSize > maxBytes) {
      chunks.push(copyFrames(bytes, group, groupSize));
      group = [];
      groupSize = 0;
    }
    group.push(frame);
  ...
  if (group.length) chunks.push(copyFrames(bytes, group, groupSize));
  return chunks;
}
...
function mp3FrameLength(bytes: Uint8Array, offset: number): number {
  ...
  if (versionBits === 1 || layerBits === 0 || bitrateIndex === 0 || bitrateIndex === 15 || sampleRateIndex === 3) return 0;
  ...
  if (layer === 1) return Math.floor(((12 * bitrate * 1000) / sampleRate + padding) * 4);
  if (layer === 3 && version !== 1) return Math.floor((72 * bitrate * 1000) / sampleRate + padding);
...
function splitWav(bytes: Uint8Array, maxBytes: number): Uint8Array[] {
  ...
  while (offset + 8 <= bytes.length) {
    ...
    if (id === "fmt " && size >= 16 && offset + 8 + size <= bytes.length) blockAlign = Math.max(1, readU16(bytes, offset + 8 + 12));
    if (id === "data") {
  ...
  const maxPayload = Math.max(blockAlign, Math.floor((maxBytes - header.length) / blockAlign) * blockAlign);
  const chunks: Uint8Array[] = [];
