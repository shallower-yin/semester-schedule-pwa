-- Keep cloud reminder expansion consistent with the client:
-- 1) an open-ended recurrence continues rolling forward;
-- 2) monthly dates clamp to the last day of short months;
-- 3) a cross-night one-off event reminds only at its start.
