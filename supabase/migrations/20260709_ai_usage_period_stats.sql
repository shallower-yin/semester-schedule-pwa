...
declare
  result jsonb;
  beijing_day_start timestamptz := date_trunc('day', now() at time zone 'Asia/Shanghai') at time zone 'Asia/Shanghai';
  beijing_month_start timestamptz := date_trunc('month', now() at time zone 'Asia/Shanghai') at time zone 'Asia/Shanghai';
begin
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      'ai_today_request_count', usage_stats.today_request_count,
      'ai_today_success_count', usage_stats.today_success_count,
      'ai_today_error_count', usage_stats.today_error_count,
      'ai_today_total_tokens', usage_stats.today_total_tokens,
      'ai_today_estimated_cost_cny', usage_stats.today_estimated_cost_cny,
      'ai_month_request_count', usage_stats.month_request_count,
      'ai_month_success_count', usage_stats.month_success_count,
      'ai_month_error_count', usage_stats.month_error_count,
      'ai_month_total_tokens', usage_stats.month_total_tokens,
      'ai_month_estimated_cost_cny', usage_stats.month_estimated_cost_cny
    )
  ...
  left join lateral (
    select
      ...
      (count(*) filter (where usage.requested_at >= beijing_day_start))::integer as today_request_count,
      (count(*) filter (where usage.requested_at >= beijing_day_start and usage.status = 'success'))::integer as today_success_count,
      (count(*) filter (where usage.requested_at >= beijing_day_start and usage.status = 'error'))::integer as today_error_count,
      coalesce(sum(usage.total_tokens) filter (where usage.requested_at >= beijing_day_start), 0)::integer as today_total_tokens,
      sum(usage.estimated_cost_cny) filter (where usage.requested_at >= beijing_day_start) as today_estimated_cost_cny,
      (count(*) filter (where usage.requested_at >= beijing_month_start))::integer as month_request_count,
      (count(*) filter (where usage.requested_at >= beijing_month_start and usage.status = 'success'))::integer as month_success_count,
      (count(*) filter (where usage.requested_at >= beijing_month_start and usage.status = 'error'))::integer as month_error_count,
      coalesce(sum(usage.total_tokens) filter (where usage.requested_at >= beijing_month_start), 0)::integer as month_total_tokens,
      sum(usage.estimated_cost_cny) filter (where usage.requested_at >= beijing_month_start) as month_estimated_cost_cny
...
begin
  ...
  select jsonb_build_object(
    ...
    'today', jsonb_build_object(
      'requestCount', (count(*) filter (where usage.requested_at >= beijing_day_start))::integer,
      'successCount', (count(*) filter (where usage.requested_at >= beijing_day_start and usage.status = 'success'))::integer,
      'errorCount', (count(*) filter (where usage.requested_at >= beijing_day_start and usage.status = 'error'))::integer,
      'totalTokens', coalesce(sum(usage.total_tokens) filter (where usage.requested_at >= beijing_day_start), 0)::integer,
      'estimatedCostCny', sum(usage.estimated_cost_cny) filter (where usage.requested_at >= beijing_day_start)
    ),
    'month', jsonb_build_object(
      'requestCount', (count(*) filter (where usage.requested_at >= beijing_month_start))::integer,
      'successCount', (count(*) filter (where usage.requested_at >= beijing_month_start and usage.status = 'success'))::integer,
      'errorCount', (count(*) filter (where usage.requested_at >= beijing_month_start and usage.status = 'error'))::integer,
      'totalTokens', coalesce(sum(usage.total_tokens) filter (where usage.requested_at >= beijing_month_start), 0)::integer,
      'estimatedCostCny', sum(usage.estimated_cost_cny) filter (where usage.requested_at >= beijing_month_start)
    )
  )
  into ai_usage
  from public.ai_assistant_usage usage
