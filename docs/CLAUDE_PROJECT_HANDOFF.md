...
- F6 弹窗编排（已提交 `298acf7`）：把五个助手/AI 弹窗（日程助手、AI 助手、思维导图、音频转写、AI 工具箱）从 `App.tsx` 抽到 `src/components/AssistantDialogs.tsx`，共享一份 `assistantInput` memo，`App.tsx` 净减约 55 行。新增 `AssistantDialogs.test.tsx` 校验按标志渲染与 props 转发；四个懒加载弹窗已用 CDP 在无头 Chrome 与 API 36 模拟器 APK 内验证可开可关。
- F7 拆分 `styles.css`（已提交 `1191154`）：采用**保序连续切分**而非按前缀重排——把原 9768 行按原顺序切成 `src/styles/01-core-schedule.css` … `06-responsive.css` 六个分层文件，原 `styles.css` 改为用 `@import` 依次引入的 barrel。因规则顺序完全不变，构建产物 CSS 与基线**逐字节相同**（hash 一致），零级联/视觉回归。这样文件可按模块导航，又规避了前缀重排的跨端风险；若日后仍要按页面前缀细拆，再有人值守用 Chrome+CDP 四视口逐页截图对比。
...
- 一加真机（ColorOS）验安全区修复与专注原生悬浮窗：模拟器是 AOSP，证不了 ColorOS 皮肤专属行为；请在你的一加上装最新调试包（`android/app/build/outputs/apk/debug/app-debug.apk`，已含本轮 F6/F7 + 原生 insets/悬浮窗，`BUILD SUCCESSFUL` 并在模拟器内 boot+弹窗冒烟通过），核验顶部不再撞状态栏、专注“系统小窗”能弹出并可拖动、点按能拉起应用。
