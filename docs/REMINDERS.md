...
已经错过超过约 15 分钟的提醒不会补发。新建测试事项时，建议把开始时间设为当前时间之后 10 分钟，并选择提前 5 分钟提醒；测试纪念日时可选择今天、提前 0 天，并把提醒时间设为几分钟后。
...
即使云端订阅失败，事项和纪念日仍可保存，并在应用保持打开时使用本地提醒；应用完全关闭后的提醒必须成功完成 Web Push 订阅。
